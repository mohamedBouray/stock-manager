...
export default function Sidebar({ currentRole, activeItem, onNavigate, collapsed, onToggle }) {
    ...
    return (
        <aside className={`h-screen bg-white border-r border-black/10 flex flex-col overflow-hidden sticky top-0 z-50 shadow-sm transition-all duration-300 ${collapsed ? 'w-[68px]' : 'w-[260px]'}`}>
            {/* Logo */}
            <div className={`border-b border-black/10 transition-all duration-300 ${collapsed ? 'px-3 py-4' : 'px-5 py-4'}`}>
                    ...
                    <div key={section.id} className="mb-5">
                        {!collapsed ? (
                            <div className="flex items-center gap-1.5 px-2 mb-2">
                                <TI name={section.icon} size={11} color="rgba(0,0,0,0.28)" />
                        ...
                        {section.items.map(item => {
                            ...
                            return (
                                ...
                                >
                                    <TI name={item.icon} size={17} color={isActive ? role.accent : 'rgba(0,0,0,0.35)'} />
